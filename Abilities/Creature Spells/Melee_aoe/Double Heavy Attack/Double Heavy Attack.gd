...
func _use():
	...
	Utility.get_node('EnemyTargetSystem')._create_circle_ability(size, cast_duration, (target.global_position - origin.global_position).normalized(), origin, callable, after_effect, 2)
